	...
	class USUARIOS_DELETE{//clase borrar
		...
		?>
			...
			<form name = 'Form' action='../Controller/USUARIOS_Controller.php' method='post' onsubmit="return comprobar_registro();">
				...
				<!-- formulario telefono -->
				<?php echo $strings['Telefono'] . ':' ?> <input type = 'text' name = 'telefono' min='0' max='999999999' id = 'telefono'  size = '9' value = "<?php echo $this->tupla['telefono']; ?>" readonly><br>

				<!-- formulario email -->
				<?php echo $strings['email'] . ':' ?>
				<input type = 'text' name = 'email' id = 'email' size = '40' value = '<?php echo $this->tupla['email']; ?>' onblur="esNoVacio('email')  && comprobarEmail('email')" readonly><br>

				<!-- formulario cumpleaños -->
				<?php echo $strings['Fecha de nacimiento'] . ':' ?> <input type = 'text' name = 'fechaNacimiento' id = 'fechaNacimiento' value= "<?php echo $this->tupla['FechaNacimiento']; ?>" readonly><br>

				<!-- formulario foto-->
				<?php echo $strings['Foto personal'] . ':'.' ' ?><input type='label' size='40' value = "<?php echo $this->tupla['fotopersonal']. ' '; ?>" readonly><br>

				<!-- formulario sexo-->
				<?php echo $strings['sexo'] . ':' ?> <input type = 'text' name = 'sexo' id = 'sexo' value = '<?php if($this->tupla['sexo']=='mujer'){ echo $strings['Mujer']; }else{ echo $strings['Hombre'];} ?>' readonly><br>
